...
export default defineConfig(({ mode }) => {
  ...
  // Ensure base path ends with / and defaults to /
  ...
  return {
    ...
    plugins: [
      ...
      {
        name: "html-base-path-rewrite",
        transformIndexHtml(html) {
          // Rewrite relative paths (./path) to use base path
          // This handles images and other assets in public folder
          // Always rewrite ./ to base path (which is at minimum /)
          html = html.replace(/(src|href|content)="\.\/([^"]+)"/g, `$1="${base}$2"`);
          return html;
        },
